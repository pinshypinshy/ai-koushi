...
**画面とサーバーが繋がり、実際に講義を作れる段階。** ログインし、教材を貼り付けて講義を
作成し、生成の進捗を見て、教材の原文を読むところまでが実データで動く。ただし受講（講義タブ）
と確認テストタブはまだモックで動いており、これらはサーバー側の実装が残っている。
...
| 受講（③講義本文・④質問応答・⑤要約）の API | 未着手。AI 呼び出し層は実装済みだが、ブラウザから叩く口が無い |
| 確認テストの出題・採点 API | 未着手 |
| 講義の削除・改名 API | 未着手。画面上でしか変わらない |
