...
**アプリとして一通り通しで使える段階。** ログインし、教材を貼り付けて講義を作成し、生成の
進捗を見て、解説を受けて質問し、ステップを進め、確認テストを受けて誤答だけを復習する、
までがローカル環境で実データにより動作する。モックデータは残っていない。

未着手は本番環境への配置（Cloudflare 上の D1 とデプロイ）と、下表の一部項目である。
...
| 講義の削除・改名 API | 実装済み |
| 本番環境（リモート D1・デプロイ） | 未着手。ローカルのみで動作する |
