...
**サーバー側は一通り揃い、画面との接続が途中まで進んだ段階。** ログインし、教材を貼り付けて
講義を作成し、生成の進捗を見て、教材の原文を読むところまでが画面から実データで動く。
受講（講義タブ）と確認テストタブは、サーバー側の受け口が揃って実データでの動作も確認できたが、
画面はまだモックのままで、両者を繋ぐ作業が残っている。
...
チャンクの間隔が保たれることを確認済み）。到達時間の短縮は画面へ繋いだ後に体感で判断する。
