...
前提：教材80,000文字 ≒ **約40,000トークン**（Q-16 の回答。2026-08-27 に `countTokens` で実測した値、2.02文字/トークンによる換算）。

| 局面 | 内訳 | 概算コスト |
|---|---|---|
| 講義作成（①骨子生成、3.1 Pro） | 入力40k（キャッシュ書込）＋ 出力2k | 約 $0.04 |
| 講義作成（②テスト生成、3.1 Pro） | 入力40k（キャッシュ読取）＋ 出力8k | 約 $0.10 |
| 講義1ターン（3.7 Flash） | 入力40k（キャッシュ読取）＋ 履歴3k ＋ 出力0.6k | 約 $0.008 |
| キャッシュ保存（3.7 Flash、1件あたり計3時間想定） | 0.04M × $0.50 × 3 | 約 $0.06 |
| **講義1件を最後まで受講** | 作成＋（35ターン）＋保存 | **約 $0.5** |

**実測の条件と限界。** 上記の 2.02文字/トークン は、日本語の技術的な散文327文字を1サンプル測定した値である。コードブロックや数式を多く含む教材では比率が変わるため、実運用の記録（`ai_usage_logs`）で継続的に確認すること。なお当初の想定は0.8文字/トークン（80,000文字 ≒ 100,000トークン）であり、実測はその約2.5倍の効率だった。コストは入力トークン数に支配されるため、概算は当初見積もりのおよそ半分になっている。

**①②の単価には未確認部分がある。** §5.5 のとおり Pro 系で実在するモデルは `gemini-3.1-pro-preview` であり、§8.2.1 の単価表は preview 版に対して検証していない。

キャッシュを適用しない場合、1ターンあたりの入力コストは $0.03（10倍）となり、講義1件あたり $1.1 前後まで膨らむ。§8.2 冒頭でコンテキストキャッシュを必須としているのはこのためである。実装後は使用量レポートでキャッシュが実際に効いていることを検証する。
...
| 月間コスト | **$15（約2,200円）** | 上表より講義8件で約$4。Flash の値上げ（Q-23）・実測比率の振れ・想定超過をまとめて吸収できる水準として据え置く |
