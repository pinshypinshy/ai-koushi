...
| 用途 | モデル | 理由 |
|---|---|---|
| ① 骨子生成 | `gemini-3.1-pro` | 教材全体の構造把握と依存関係の判断が必要で、失敗コストが最も高い |
| ② 確認テスト生成 | `gemini-3.1-pro` | ディストラクタ品質（§5.3 R-3）が成果物の価値を左右するため |
...
| AI SDK | Google 公式 TypeScript SDK（`@google/genai`） | fetch ベースで Workers 上から利用できる。構造化出力・ストリーミング・コンテキストキャッシュを直接扱える。**［要確認］（Q-22）** Workers 上での動作は実装初期に確認する |
| 認証 | **［要確認］（Q-21）** Google OAuth（§4.6）を Workers 上でどう実装するか。Auth.js（`@auth/core`）等の Web標準ベースのライブラリを想定 | — |
...
前提：教材80,000文字 ≒ 約100,000トークン（**［要確認］（Q-16）実装時に実測して補正すること**）。

| 局面 | 内訳 | 概算コスト |
|---|---|---|
| 講義作成（①骨子生成、3.1 Pro） | 入力100k（キャッシュ書込）＋ 出力2k | 約 $0.06 |
| 講義作成（②テスト生成、3.1 Pro） | 入力100k（キャッシュ読取）＋ 出力8k | 約 $0.12 |
| 講義1ターン（3.7 Flash） | 入力100k（キャッシュ読取）＋ 履歴3k ＋ 出力0.6k | 約 $0.012 |
| キャッシュ保存（3.7 Flash、1件あたり計3時間想定） | 0.1M × $0.50 × 3 | 約 $0.15 |
| **講義1件を最後まで受講** | 作成＋（35ターン）＋保存 | **約 $0.9** |

キャッシュを適用しない場合、1ターンあたりの入力コストは $0.075（10倍）となり、講義1件あたり $2.6 前後まで膨らむ。§8.2 冒頭でコンテキストキャッシュを必須としているのはこのためである。実装後は使用量レポートでキャッシュが実際に効いていることを検証する。
...
| 月間コスト | **$15（約2,200円）** | 上表より講義8件で約$7.2。Flash の値上げ（Q-23）と想定超過の双方を吸収できる水準として設定 |
...
| Q-22 | §7.1 | `@google/genai` が Cloudflare Workers 上で動作するかの確認 | AI層実装前 |
