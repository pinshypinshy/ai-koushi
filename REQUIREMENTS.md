...
**番号について**：Q-番号は検討の順に付与しており、欠番はない。Q-24 は初版（v0.1）の確定後に追加した論点である。ただし Q-5 は当初「1ステップあたりの問題数」を問うていたが、回答によって「設問とステップの対応関係」へ論点が移ったため、番号と当初の問いが一致しない。上表の「内容」列が現在の論点である。
